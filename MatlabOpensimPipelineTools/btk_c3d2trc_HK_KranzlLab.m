function data = btk_c3d2trc_HK_KranzlLab(varargin)
% function btk_c3d2trc(file) OR
% function btk_c3d2trc(data)
%
% Function to convert data from a C3D file into the TRC and MOT file
% formats for OpenSim
% saves TRC and MOT file in the same folder as the original file
%%%%%%%%%%%%%%%
% input lab (Kranzl): x anterior, y medial/lateral, z up
% if different change following lines in the script
% 124-146 and 237-250
%%%%%%%%%%%%%%%%
...
% Modified by Hans Kainz (KU Leuven) September 2017
% rotates data if person is walking in -x direction
%% load data
if nargin > 0
...
% change depending on the global orientation of the gait laboratory
% go through each marker field and re-order from X Y Z to X Z Y
for i = 1:nmarkers
   data.marker_data.Markers.(markers{i}) =  [data.marker_data.Markers.(markers{i})(:,1)...
       data.marker_data.Markers.(markers{i})(:,3) (data.marker_data.Markers.(markers{i})(:,2).*(-1))]/p_sc;
end
...
midASIS = (data.marker_data.Markers.LASI + data.marker_data.Markers.RASI)./2;
if midASIS(1,1) > midASIS(50,1) % person is walking -x direction
    % rotate marker trajectories
    orientation.dir_front   = [ -1 0 0];%  walking in -x direction 
    orientation.dir_up      = [ 0 1 0];%
    RotationMatrix = [orientation.dir_front; orientation.dir_up; cross(orientation.dir_front,orientation.dir_up)];
    
    for i = 1:nmarkers
        data.marker_data.Markers.(markers{i}) =  (RotationMatrix*data.marker_data.Markers.(markers{i})')'
    end
...
for i = 1:length(data.fp_data.FP_data)
    % change depending on the global frame of the gait laboratory
   ...
   data.fp_data.GRF_data(i).P =  [data.fp_data.GRF_data(i).P(:,1)/p_sc ...
       data.fp_data.GRF_data(i).P(:,3)/p_sc (data.fp_data.GRF_data(i).P(:,2).*(-1))/p_sc];  
   
    if midASIS(1,1) > midASIS(50,1) % person is walking -x direction
       data.fp_data.GRF_data(i).P =  (RotationMatrix*data.fp_data.GRF_data(i).P')'       
    else
    end
   
   
   data.fp_data.GRF_data(i).F =  [data.fp_data.GRF_data(i).F(:,1) ...
       data.fp_data.GRF_data(i).F(:,3) data.fp_data.GRF_data(i).F(:,2)];
   data.fp_data.GRF_data(i).M =  [data.fp_data.GRF_data(i).M(:,1) ...
       data.fp_data.GRF_data(i).M(:,3) data.fp_data.GRF_data(i).M(:,2)]/p_sc;
