...
for trialIndex = 1 : size(trialsFileNames, 2)
    ...
    for modelIndex = 1 : size(scaledModelsFileNames, 2)
        try
            ...
            if(b_runIK == 1)
                resultsAreValid = runIK_TimeFrameOfInterest(setupIKfile, modelFile, trcFile, startTime, endTime, fullfile(outputPath, 'IK'));
                if(~resultsAreValid)
            ...
            if(b_runID == 1)
                runID_TimeFrameOfInterest(setupIDfile, modelFile, motionFile, grfSetupFile, startTime, endTime, fullfile(outputPath, 'ID'));
            end
            ...
            if(b_runSO == 1)
                runSO_TimeFrameOfInterest(setupSOfile, actuatorfile, modelFile, motionFile, grfSetupFile, startTime, endTime, fullfile(outputPath, 'SO'));
            end
            ...
            if(b_runJRL == 1)
                ...
                runJRL_TimeFrameOfInterest(setupJRLfile, actuatorfile, modelFile, motionFile, grfSetupFile, so_forcesFile, startTime, endTime, fullfile(outputPath, 'JRL'));
            end
