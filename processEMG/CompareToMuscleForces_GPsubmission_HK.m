...
for sm=1:length(ScaleMethod)
    for m=1:length(muscle)
        Force_summary.Max.All_subj.(ScaleMethod{sm}).AnkleDF(m,:)=max(Force_summary.All_subj.(ScaleMethod{sm}).(muscle{m}));
    end
end


for sm=1:length(ScaleMethod)
    Force_summary.Max.All_subj.AnkleDF_meanAllMuscles(sm,:)=mean(Force_summary.Max.All_subj.(ScaleMethod{sm}).AnkleDF);
end
% cd('Y:\MATLABcodes\Hans_DATA\');
