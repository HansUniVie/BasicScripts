function DOMnode = xml_writeOSIM(filename, tree, RootName, Pref)
...
% except for F_xmlwrite function which was Alberto Amaro's correction to
% xmlwrite MATLAB function. 

%   Copyright 2007 Jarek Tuszynski - SAIC

% Modify by Morgan Sangeux - Royal Children's Hospital Melbourne - 2009
% Specific feature were needed for OpenSim model file writting
% Muscle points have to appear in a certain order so can not use the
% default by category organization
...
DPref.XmlEngine  = 'Xerces';  % use matlab provided XMLUtils
...
%  Pref - preferenced
  ...
  elseif (isstruct(s))  % if struct than deal with each field separatly
    ...
    %============ Special treatment for MusclePoints in OpenSim files =====
    % if order is one of the field then can not treat by category
    if strcmp(name,'objects') && isfield(s,'order')
        ObjectsOrder = s.order;
        s = rmfield(s,'order');
        % s should not be an array of struct
        node = xml.createElement(name);
        for o = 1:length(ObjectsOrder(:,1))
            family = s.(ObjectsOrder{o,2});
            x = family{ObjectsOrder{o,3}};
            struct2DOMnode(xml, node, x, ObjectsOrder{o,2}, Pref ); % recursive call will modify 'node'
        end
        parent.appendChild(node);    
    ...
    elseif (strcmp(name, 'PROCESSING_INSTRUCTION')) % set attributes of the node
      ...
      if (~OK)
        warning(['Struct field named PROCESSING_INSTRUCTION need to be',...
...
if (~isempty(p))
   for i=1:length(p)
     ...
     str = regexprep(str,before,after, 'once', 'preservecase');  
...
str = regexprep(str,'xFunction','function'  ,'once', 'preservecase');
