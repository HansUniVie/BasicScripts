function [marker_data, analog_data, sub_info, angle_data] = btk_loadc3d_cp(file, grw_threshold)
...
%           grw_threshold - the force threshold for calculating the GRW
%           (default 5 N)
%
...
if nargout > 1
    ...
    if nargout > 2
        ...
        if nargout>3
            ...
            % get processed angle data
            angles = btkGetAngles(acq);
            angle_data.angles = angles;
            
            
%             angle_values = btkGetAnglesValues(acq);
%             angle_data.angle_values = angle_values;            
        end
