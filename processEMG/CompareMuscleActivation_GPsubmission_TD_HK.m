...
for sm=1:length(ScaleMethod)
    for m=1:length(muscle)
        Activation_summary.Max.All_subj.(ScaleMethod{sm}).AnklePF(m,:)=max(Activation_summary.All_subj.(ScaleMethod{sm}).(muscle{m}));
    end
end


for sm=1:length(ScaleMethod)
    Activation_summary.Max.All_subj.AnklePF_meanAllMuscles(sm,:)=mean(Activation_summary.Max.All_subj.(ScaleMethod{sm}).AnklePF);
end
% cd('Y:\MATLABcodes\Hans_DATA\');
