...
PI = {'TD01','TD02','TD03','TD04','TD05','TD07'}; %participant index (intervention + number)

dir_main = 'C:\Users\u0113767\Documents\WORK_KULeuven\ClinicalMuscleForce_project\Data_PellenbergStrenghProject\TD\';
...
for sub=1:length(PI)
    ...
    SO_left = [];
    SO_right =[];
    SO_left = load([dir_main (PI{sub}) '\TD_CapGap\SOresults_l.mat']);
    SO_right = load([dir_main (PI{sub}) '\TD_CapGap\SOresults_r.mat']);
...
for sm=1:length(ScaleMethod)
    for m=1:length(muscle)
        Force_summary.Max.All_subj.(ScaleMethod{sm}).KneeFlext(m,:)=max(Force_summary.All_subj.(ScaleMethod{sm}).(muscle{m}));
    end
end


for sm=1:length(ScaleMethod)
    Force_summary.Max.All_subj.KneeFlext_meanAllMuscles(sm,:)=mean(Force_summary.Max.All_subj.(ScaleMethod{sm}).KneeFlext);
end
% cd('Y:\MATLABcodes\Hans_DATA\');
